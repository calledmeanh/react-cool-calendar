...
;// CONCATENATED MODULE: ./src/model/calendar.ts
var EAction = /*#__PURE__*/function (EAction) {
  EAction["CHANGE_MODE"] = "CHANGE_MODE";
  EAction["CHANGE_ZOOM"] = "CHANGE_ZOOM";
  EAction["PREV_DAY"] = "PREV_DAY";
...
var EViewMode = /*#__PURE__*/function (EViewMode) {
  EViewMode["LARGE"] = "LARGE";
  EViewMode["SMALL"] = "SMALL";
  return EViewMode;
}({});
var EStatus = /*#__PURE__*/function (EStatus) {
  EStatus["BOOKED"] = "BOOKED";
  EStatus["CONFIRMED"] = "CONFIRMED";
  EStatus["ARRIVED"] = "ARRIVED";
  EStatus["STARTED"] = "STARTED";
  EStatus["NOSHOW"] = "NOWSHOW";
  return EStatus;
}({});
;// CONCATENATED MODULE: ./src/model/index.ts


;// CONCATENATED MODULE: ./src/mock/appointment.ts
...
;// CONCATENATED MODULE: ./src/constant/config.ts
var SECONDS_PER_MINUTE = 60;
var SECONDS_PER_HOUR = 3600;
var HAFT_DAY_SECONDS = 12 * SECONDS_PER_HOUR - 1; // 11h59m59s

var ZOOMMODE_LARGE = 15 * SECONDS_PER_MINUTE;
var ZOOMMODE_SMALL = 30 * SECONDS_PER_MINUTE;
var MAPPING_TIME = {
  900: 4,
  1800: 2
};
...
var CONFIG = {
  ...
  HAFT_DAY_SECONDS: HAFT_DAY_SECONDS,
  ZOOMMODE_LARGE: ZOOMMODE_LARGE,
  ZOOMMODE_SMALL: ZOOMMODE_SMALL,
...
var TimeUtils = {
  calcDistanceBetweenTimes: calcDistanceBetweenTimes,
  calcTimeStep: calcTimeStep,
  createTimes: createTimes,
  checkGroupTime: checkGroupTime,
  displayTime: displayTime,
  checkWorkingTime: checkWorkingTime,
  convertHeightToDuration: convertHeightToDuration,
  covertHourMinuteToSeconds: covertHourMinuteToSeconds,
  covertSecondsToHourMinute: covertSecondsToHourMinute,
  convertSecondsToHourString: convertSecondsToHourString,
  convertMinuteToSeconds: convertMinuteToSeconds,
  parseDurationToViewMode: parseDurationToViewMode,
  wrapperSetInterval: wrapperSetInterval,
  wrapperSetTimeout: wrapperSetTimeout
};

/* calculate top position based-on time */
function calcDistanceBetweenTimes(end, start, duration, lineHeight) {
  var jumps = calcTimeStep(end, start, duration);
  var height = jumps * lineHeight;
  var rest = end - (duration * jumps + start);
  var restHeight = rest * lineHeight / duration;
  return height + restHeight;
}

/* calculate total steps based-on start and end time */
function calcTimeStep(end, start, duration) {
  var S = end - start;
  var step = S / duration;
  return Math.floor(step);
}
function createTimes(end, start, duration) {
  var timeStep = calcTimeStep(end, start, duration);
  var times = new Array(timeStep).fill(0);
  return times;
}

/* get something like that --> 00:00 AM */
function convertSecondsToHourString(seconds) {
  var timeType = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 24;
  var tempSeconds = seconds;
  if (timeType === 12 && seconds >= 13 * CONFIG.SECONDS_PER_HOUR) {
    tempSeconds = seconds - 12 * CONFIG.SECONDS_PER_HOUR;
  }
  var time = covertSecondsToHourMinute(Math.abs(tempSeconds));
  var timeString = "".concat(formatHourOrMinute(time.hour), ":").concat(formatHourOrMinute(time.minute));
  if (timeType === 12) {
    timeString = "".concat(timeString, " ").concat(seconds <= CONFIG.HAFT_DAY_SECONDS ? "AM" : "PM");
  }
  return timeString;
}
function covertSecondsToHourMinute(seconds) {
  var hour = Math.floor(seconds / CONFIG.SECONDS_PER_HOUR);
  var minute = (seconds - hour * CONFIG.SECONDS_PER_HOUR) / CONFIG.SECONDS_PER_MINUTE;
  return {
    hour: hour,
    minute: Math.floor(minute)
  };
}
function formatHourOrMinute(data) {
  return ("0" + data).slice(-2);
}
function convertHeightToDuration(height, lineHeight, duration) {
  return height / lineHeight * (duration / CONFIG.SECONDS_PER_MINUTE);
}
function covertHourMinuteToSeconds(hour, minute) {
...
function displayTime(current, start, duration) {
  return (current - start) % duration === 0 ? true : false;
}
function checkWorkingTime(dayTime, workingTime, current) {
...
function checkGroupTime(groupTimeDuration, duration, timeJumpIndex) {
  var groupBy = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : "bottom";
  var groupStep = groupTimeDuration / duration;
  var index = timeJumpIndex + 1;
  if (groupBy === "top") {
    index = timeJumpIndex;
  }
...
function parseDurationToViewMode(duration) {
  var res = "";
  switch (duration) {
...
function wrapperSetInterval(handler, time) {
  var intervalId = setInterval(handler, time);
  return function () {
    clearInterval(intervalId);
  };
}
function wrapperSetTimeout(handler, time) {
  var intervalId = setTimeout(handler, time);
  return function () {
    clearTimeout(intervalId);
  };
}
;// CONCATENATED MODULE: ./src/util/clsx.ts
function clsx(props) {
  var string = "";
...
function getPrevDay(currentDate) {
  return dayjs_min_default()(currentDate).subtract(1, "d");
}
function getNextDay(currentDate) {
  return dayjs_min_default()(currentDate).add(1, "d");
}
function getPrevWeek(currentDate) {
  return dayjs_min_default()(currentDate).subtract(7, "d");
}
...
function getCustomDateToDate(currentDate) {
  var startOrigin = currentDate.startOf("weeks"); // monday
  var start = {
    date: startOrigin.format("D"),
    month: startOrigin.format("MMM")
  };
  var endOrigin = currentDate.endOf("weeks"); // sunday
  var end = {
  ...
  if (start.month === end.month) {
    res = "".concat(start.date, " - ").concat(end.date, " ").concat(end.month, ", ").concat(end.year, " ");
  } else {
    res = "".concat(start.date, " ").concat(start.month, " - ").concat(end.date, " ").concat(end.month, ", ").concat(end.year, " ");
  }
  return res;
}
function getCustomDay(currentDate) {
  var number = currentDate.format("D");
  var text = currentDate.format("dddd");
  var month = currentDate.format("MMM");
  var year = currentDate.format("YYYY");
  var res = "".concat(text, " ").concat(number, " ").concat(month, ", ").concat(year);
  return res;
}
function isEqual(dayA, dayB) {
  var dayAStr = dayA.format(CONFIG.DATE_FORMAT);
...
var ConfigUtils = {
  createExampleCalendarState: createExampleCalendarState
};
function createExampleCalendarState() {
  var state = {
    duration: 0,
    timeType: 0,
    groupTime: 0,
    ...
    todayGlobalIns: dayjs_min_default()(),
    currentDate: dayjs_min_default()(),
    isFireEvent: false
  };
  return Object.create(state);
}
;// CONCATENATED MODULE: ./src/util/element.ts

var ElementUtils = {
  getValueFromUserInput: getValueFromUserInput,
  getOffsetToDocument: getOffsetToDocument,
  getParentNodeFrom: getParentNodeFrom
};

/**
 * get value from input or select element
 */
function getValueFromUserInput(e) {
  return e && e.target ? e.target.value : "";
}
function getOffsetToDocument(elem, dir) {
  var offset = dir === "left" ? "offsetLeft" : "offsetTop";
  var value = 0;
  do {
...
function getParentNodeFrom(elem, targetData) {
  var target = null;
  do {
    var dataIdtf = elem.getAttribute(CONFIG.DATA_IDTF.THIS);
...
;// CONCATENATED MODULE: ./src/util/appointment.ts
...
var AppointmentUtils = {
  layoutAlgorithm: layoutAlgorithm,
  getApptColorByStatus: getApptColorByStatus
};
 ...
 */
function layoutAlgorithm(appts, opts) {
  var columns = [];
  var lastEventEnding = null;
  var events = appts.map(function (appt) {
    return _objectSpread(_objectSpread(_objectSpread({}, appt), calcApptPos(appt, _objectSpread({}, opts))), {}, {
      width: 0,
      left: 0,
      weekcolumnIndex: opts.weekcolumnIndex
    });
  });
  ...
  events.forEach(function (e) {
    if (lastEventEnding !== null && e.top >= lastEventEnding) {
      packEvents(columns, _objectSpread({}, opts));
      columns = [];
...
function packEvents(columnAppt, otps) {
  var n = columnAppt.length;
  for (var i = 0; i < n; i++) {
    var col = columnAppt[i];
    for (var j = 0; j < col.length; j++) {
      // cell.l eft = ((i / n) * 100) / datelineLength;
      var cell = col[j];
      var cellWidth = otps.columnWidth / n - 1;
      ...
      var cellLeft = cellWidth * i + i + otps.columnWidth * otps.weekcolumnIndex;
      cell.left = cellLeft;
...
function calcApptPos(appt, opts) {
  var apptEndTime = appt.startTime + TimeUtils.convertMinuteToSeconds(appt.duration);
  var top = TimeUtils.calcDistanceBetweenTimes(appt.startTime, opts.daytimeStart, opts.duration, CONFIG.CSS.LINE_HEIGHT);
  var mapDuration = CONFIG.MAPPING_TIME[opts.duration];
  var height = TimeUtils.calcDistanceBetweenTimes(apptEndTime, appt.startTime, mapDuration, CONFIG.CSS.LINE_HEIGHT * mapDuration / opts.duration);
  return {
    top: top,
    height: height - 1,
...
function checkOverlapped(a, b) {
  var start = a.top;
  var otherStart = b.top;
  var end = start + a.height;
  var otherEnd = otherStart + b.height;
  if (start < otherStart && otherStart < end || start < otherEnd && otherEnd < end || otherStart < start && start < otherEnd || otherStart < end && end < otherEnd || otherStart === start && otherEnd === end) {
    return true;
  }
  return false;
}
function getApptColorByStatus(color) {
  switch (color) {
    case EStatus.BOOKED:
...
;// CONCATENATED MODULE: ./src/hook/useCalendarContext.tsx
function useCalendarContext_typeof(o) { "@babel/helpers - typeof"; return useCalendarContext_typeof = "function" == typeof Symbol && "symbol" == typeof Symbol.iterator ? function (o) { return typeof o; } : function (o) { return o && "function" == typeof Symbol && o.constructor === Symbol && o !== Symbol.prototype ? "symbol" : typeof o; }, useCalendarContext_typeof(o); }
function useCalendarContext_ownKeys(e, r) { var t = Object.keys(e); if (Object.getOwnPropertySymbols) { var o = Object.getOwnPropertySymbols(e); r && (o = o.filter(function (r) { return Object.getOwnPropertyDescriptor(e, r).enumerable; })), t.push.apply(t, o); } return t; }
function useCalendarContext_objectSpread(e) { for (var r = 1; r < arguments.length; r++) { var t = null != arguments[r] ? arguments[r] : {}; r % 2 ? useCalendarContext_ownKeys(Object(t), !0).forEach(function (r) { useCalendarContext_defineProperty(e, r, t[r]); }) : Object.getOwnPropertyDescriptors ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(t)) : useCalendarContext_ownKeys(Object(t)).forEach(function (r) { Object.defineProperty(e, r, Object.getOwnPropertyDescriptor(t, r)); }); } return e; }
function useCalendarContext_defineProperty(obj, key, value) { key = useCalendarContext_toPropertyKey(key); if (key in obj) { Object.defineProperty(obj, key, { value: value, enumerable: true, configurable: true, writable: true }); } else { obj[key] = value; } return obj; }
function useCalendarContext_toPropertyKey(arg) { var key = useCalendarContext_toPrimitive(arg, "string"); return useCalendarContext_typeof(key) === "symbol" ? key : String(key); }
function useCalendarContext_toPrimitive(input, hint) { if (useCalendarContext_typeof(input) !== "object" || input === null) return input; var prim = input[Symbol.toPrimitive]; if (prim !== undefined) { var res = prim.call(input, hint || "default"); if (useCalendarContext_typeof(res) !== "object") return res; throw new TypeError("@@toPrimitive must return a primitive value."); } return (hint === "string" ? String : Number)(input); }
...
var CalendarContext = /*#__PURE__*/(0,react.createContext)(ConfigUtils.createExampleCalendarState());
var CalendarDispatchContext = /*#__PURE__*/(0,react.createContext)(function (value) {});
var useCalendarState = function useCalendarState() {
  return (0,react.useContext)(CalendarContext);
};
var useCalendarDispatch = function useCalendarDispatch() {
  return (0,react.useContext)(CalendarDispatchContext);
};
/**
...
var CalendarProvider = function CalendarProvider(_ref) {
  ...
  var _useReducer = (0,react.useReducer)(calendarReducer, initialState),
    _useReducer2 = _slicedToArray(_useReducer, 2),
    state = _useReducer2[0],
    dispatch = _useReducer2[1];
  return /*#__PURE__*/react.createElement(CalendarContext.Provider, {
    value: state
  }, /*#__PURE__*/react.createElement(CalendarDispatchContext.Provider, {
    value: dispatch
  }, children));
};
...
/* harmony default export */ const useCalendarContext = (CalendarProvider);
;// CONCATENATED MODULE: ./src/app/common/Flex.ts
var _templateObject;
...
/* harmony default export */ const app_Zoom = (Zoom);
;// CONCATENATED MODULE: ./src/app/DateManipulation.tsx
...
var DateManipulation = function DateManipulation() {
  var calendarState = useCalendarState();
  var dispath = useCalendarDispatch();
  ...
  var onToday = function onToday(e) {
    dispath({
      type: EAction.GET_TODAY,
      payload: calendarState.todayGlobalIns
    });
  };
...
var Dateline = function Dateline() {
  ...
  var render = (0,react.useCallback)(function () {
    ...
    return dateline.map(function (d, i) {
      var isToday = DateUtils.isEqual(d.origin, calendarState.todayGlobalIns);
      var classname = clsx({
        today: isToday
      });
      return /*#__PURE__*/react.createElement(DatelineHeader, {
        key: i,
        $justify: "center",
        $align: "center",
  ...
  return /*#__PURE__*/react.createElement(Dateline_Wrapper, {
    "data-idtf": CONFIG.DATA_IDTF.DATELINE
  }, render());
};
/* harmony default export */ const app_Dateline = (Dateline);
;// CONCATENATED MODULE: ./src/app/NowIndicator.tsx
var NowIndicator_templateObject, NowIndicator_templateObject2;
function NowIndicator_slicedToArray(arr, i) { return NowIndicator_arrayWithHoles(arr) || NowIndicator_iterableToArrayLimit(arr, i) || NowIndicator_unsupportedIterableToArray(arr, i) || NowIndicator_nonIterableRest(); }
function NowIndicator_nonIterableRest() { throw new TypeError("Invalid attempt to destructure non-iterable instance.\nIn order to be iterable, non-array objects must have a [Symbol.iterator]() method."); }
function NowIndicator_unsupportedIterableToArray(o, minLen) { if (!o) return; if (typeof o === "string") return NowIndicator_arrayLikeToArray(o, minLen); var n = Object.prototype.toString.call(o).slice(8, -1); if (n === "Object" && o.constructor) n = o.constructor.name; if (n === "Map" || n === "Set") return Array.from(o); if (n === "Arguments" || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(n)) return NowIndicator_arrayLikeToArray(o, minLen); }
function NowIndicator_arrayLikeToArray(arr, len) { if (len == null || len > arr.length) len = arr.length; for (var i = 0, arr2 = new Array(len); i < len; i++) arr2[i] = arr[i]; return arr2; }
function NowIndicator_iterableToArrayLimit(r, l) { var t = null == r ? null : "undefined" != typeof Symbol && r[Symbol.iterator] || r["@@iterator"]; if (null != t) { var e, n, i, u, a = [], f = !0, o = !1; try { if (i = (t = t.call(r)).next, 0 === l) { if (Object(t) !== t) return; f = !1; } else for (; !(f = (e = i.call(t)).done) && (a.push(e.value), a.length !== l); f = !0); } catch (r) { o = !0, n = r; } finally { try { if (!f && null != t["return"] && (u = t["return"](), Object(u) !== u)) return; } finally { if (o) throw n; } } return a; } }
function NowIndicator_arrayWithHoles(arr) { if (Array.isArray(arr)) return arr; }
function NowIndicator_taggedTemplateLiteral(strings, raw) { if (!raw) { raw = strings.slice(0); } return Object.freeze(Object.defineProperties(strings, { raw: { value: Object.freeze(raw) } })); }
...
var NowIndicator = function NowIndicator(_ref) {
  var type = _ref.type,
    _ref$widthTimeline = _ref.widthTimeline,
    widthTimeline = _ref$widthTimeline === void 0 ? 0 : _ref$widthTimeline;
  var calendarState = useCalendarState();
  var _useState = (0,react.useState)({
      position: 0,
      text: "00:00"
    }),
    _useState2 = NowIndicator_slicedToArray(_useState, 2),
    now = _useState2[0],
    setNow = _useState2[1];
  var _useState3 = (0,react.useState)({
    ...
    _useState4 = NowIndicator_slicedToArray(_useState3, 2),
    timelinePos = _useState4[0],
    setTimelinePos = _useState4[1];
  var getCurrentTime = (0,react.useCallback)(function () {
    var currentHours = calendarState.todayGlobalIns.hour();
    var currentMinutes = calendarState.todayGlobalIns.minute();
    var res = TimeUtils.covertHourMinuteToSeconds(currentHours, currentMinutes);
    return res;
  }, [calendarState.todayGlobalIns]);
  var checkToday = (0,react.useCallback)(function () {
    var res = DateUtils.isEqual(calendarState.currentDate, calendarState.todayGlobalIns);
    return res;
  }, [calendarState.currentDate, calendarState.todayGlobalIns]);
  var updateEachInterval = (0,react.useCallback)(function (currentTime, today) {
    if (currentTime < calendarState.dayTime.end && currentTime > calendarState.dayTime.start && today) {
      var dateEachInterval = dayjs_min_default()();
      var hourEachInterval = dateEachInterval.hour();
      var minuteEachInterval = dateEachInterval.minute();
      var timeEachInterval = TimeUtils.covertHourMinuteToSeconds(hourEachInterval, minuteEachInterval);
      var position = TimeUtils.calcDistanceBetweenTimes(timeEachInterval, calendarState.dayTime.start, calendarState.duration, CONFIG.CSS.LINE_HEIGHT);
      var text = TimeUtils.convertSecondsToHourString(timeEachInterval, calendarState.timeType);
      setNow({
        position: position,
        text: text
      });
  ...
  var getTimelineNowPos = (0,react.useCallback)(function () {
    var dateline = DateUtils.getDateline(calendarState.currentDate, calendarState.viewMode);
    var todayIdx = dateline.findIndex(function (d) {
      return DateUtils.isEqual(d.origin, calendarState.todayGlobalIns);
    });
    if (todayIdx > -1) {
      setTimelinePos({
        width: widthTimeline,
        left: widthTimeline * todayIdx
      });
    }
  ...
  (0,react.useEffect)(function () {
    var currentTime = getCurrentTime();
    var isToday = checkToday();
    var source = setInterval(function () {
      updateEachInterval(currentTime, isToday);
    }, 30000);
    return function () {
  ...
  return /*#__PURE__*/react.createElement(react.Fragment, null, type === "PILL" && calendarState.viewMode === "DAY" && Boolean(now.position) && calendarState.nowIndicator && /*#__PURE__*/react.createElement(TimePill, {
    $align: "center",
    $justify: "center",
    $top: now.position
  }, now.text), type === "LINE" && Boolean(now.position) && calendarState.nowIndicator && /*#__PURE__*/react.createElement(TimeLine, {
    $top: now.position + 9.5,
...
function ApptBooking_toPropertyKey(arg) { var key = ApptBooking_toPrimitive(arg, "string"); return ApptBooking_typeof(key) === "symbol" ? key : String(key); }
function ApptBooking_toPrimitive(input, hint) { if (ApptBooking_typeof(input) !== "object" || input === null) return input; var prim = input[Symbol.toPrimitive]; if (prim !== undefined) { var res = prim.call(input, hint || "default"); if (ApptBooking_typeof(res) !== "object") return res; throw new TypeError("@@toPrimitive must return a primitive value."); } return (hint === "string" ? String : Number)(input); }
function ApptBooking_slicedToArray(arr, i) { return ApptBooking_arrayWithHoles(arr) || ApptBooking_iterableToArrayLimit(arr, i) || ApptBooking_unsupportedIterableToArray(arr, i) || ApptBooking_nonIterableRest(); }
function ApptBooking_nonIterableRest() { throw new TypeError("Invalid attempt to destructure non-iterable instance.\nIn order to be iterable, non-array objects must have a [Symbol.iterator]() method."); }
function ApptBooking_unsupportedIterableToArray(o, minLen) { if (!o) return; if (typeof o === "string") return ApptBooking_arrayLikeToArray(o, minLen); var n = Object.prototype.toString.call(o).slice(8, -1); if (n === "Object" && o.constructor) n = o.constructor.name; if (n === "Map" || n === "Set") return Array.from(o); if (n === "Arguments" || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(n)) return ApptBooking_arrayLikeToArray(o, minLen); }
function ApptBooking_arrayLikeToArray(arr, len) { if (len == null || len > arr.length) len = arr.length; for (var i = 0, arr2 = new Array(len); i < len; i++) arr2[i] = arr[i]; return arr2; }
function ApptBooking_iterableToArrayLimit(r, l) { var t = null == r ? null : "undefined" != typeof Symbol && r[Symbol.iterator] || r["@@iterator"]; if (null != t) { var e, n, i, u, a = [], f = !0, o = !1; try { if (i = (t = t.call(r)).next, 0 === l) { if (Object(t) !== t) return; f = !1; } else for (; !(f = (e = i.call(t)).done) && (a.push(e.value), a.length !== l); f = !0); } catch (r) { o = !0, n = r; } finally { try { if (!f && null != t["return"] && (u = t["return"](), Object(u) !== u)) return; } finally { if (o) throw n; } } return a; } }
function ApptBooking_arrayWithHoles(arr) { if (Array.isArray(arr)) return arr; }
function ApptBooking_taggedTemplateLiteral(strings, raw) { if (!raw) { raw = strings.slice(0); } return Object.freeze(Object.defineProperties(strings, { raw: { value: Object.freeze(raw) } })); }
...
var ApptBooking = function ApptBooking(_ref) {
  ...
  var value = _ref.value,
    ...
    widthTimeline = _ref.widthTimeline,
    onPressAppt = _ref.onPressAppt,
    onReleaseAppt = _ref.onReleaseAppt;
  var calendarState = useCalendarState();
  var dispath = useCalendarDispatch();
  ...
  var _useState = (0,react.useState)({
    ...
    _useState2 = ApptBooking_slicedToArray(_useState, 2),
  ...
  var _useState3 = (0,react.useState)({
    ...
    _useState4 = ApptBooking_slicedToArray(_useState3, 2),
    size = _useState4[0],
    setSize = _useState4[1];
  var isDragRef = (0,react.useRef)(false);
  var isResizeRef = (0,react.useRef)(false);
  var origDeltaXRef = (0,react.useRef)(0);
  var origDeltaYRef = (0,react.useRef)(0);
  var lastMouseTopPositionRef = (0,react.useRef)(0);
  var sizeBackerRef = (0,react.useRef)(0);
  var topEdgeRef = (0,react.useRef)(0);
  var calendarRef = (0,react.useRef)(null);
  var removeAutoScrollIntervalRef = (0,react.useRef)(function () {});
  var preventDragEventRef = (0,react.useRef)(function () {});
  var autoScrollThresholdRef = (0,react.useRef)(value.height / 5); // threshold to start auto scroll

  var floorTop = Math.floor(mousePosition.pageY / CONFIG.CSS.LINE_HEIGHT) * CONFIG.CSS.LINE_HEIGHT;
  var lineIdx = position.top / CONFIG.CSS.LINE_HEIGHT;
  var startTime = lineIdx * calendarState.duration + calendarState.dayTime.start;
  var endTimeByDragging = startTime + (value.height + 1) * calendarState.duration / CONFIG.CSS.LINE_HEIGHT;
  var endTimeByResizing = startTime + (size.height + 1) * calendarState.duration / CONFIG.CSS.LINE_HEIGHT;
  var newStartTime = isDragRef.current ? startTime : value.startTime;
  var newEndTime = isDragRef.current ? endTimeByDragging : isResizeRef.current ? endTimeByResizing : value.endTime;
  var updatedStartTime = TimeUtils.convertSecondsToHourString(newStartTime);
  var updatedEndTime = TimeUtils.convertSecondsToHourString(newEndTime);
  var updatedWidth = isDragRef.current ? widthTimeline : size.width;
  var updatedLeft = isDragRef.current ? mousePosition.left : position.left;
  var updatedHeight = isResizeRef.current ? size.height : value.height;
  var calendarHeight = ((_calendarRef$current = calendarRef.current) === null || _calendarRef$current === void 0 ? void 0 : _calendarRef$current.offsetHeight) || 0;
  var scrollBarHeight = (scrollEl === null || scrollEl === void 0 ? void 0 : scrollEl.offsetHeight) || 0;
  var maxScrollTop = (scrollEl === null || scrollEl === void 0 ? void 0 : scrollEl.scrollHeight) || 0;

  // distance from mouse to
  var distanceUp = mousePosition.top - origDeltaYRef.current;
  var distanceLeft = mousePosition.left - origDeltaXRef.current;
  var distanceDown = mousePosition.top + (value.height - origDeltaYRef.current);
  var steps = TimeUtils.calcTimeStep(calendarState.dayTime.end, calendarState.dayTime.start, calendarState.duration);
  var maxGridHeight = steps * CONFIG.CSS.LINE_HEIGHT;

  /* handle drag event */
  var updateDraggingState = function updateDraggingState(value) {
    if (value) {
      isDragRef.current = value;
      dispath({
        type: EAction.UPDATE_FIRE_EVENT,
        payload: value
      });
    } else {
      isDragRef.current = value;
  ...
  var handleMouseOutsideWhileDragging = function handleMouseOutsideWhileDragging(e) {
    e.stopPropagation();
  ...
  var onStartDragging = function onStartDragging(e) {
    preventDragEventRef.current = TimeUtils.wrapperSetTimeout(function () {
    ...
    calendarRef.current = ElementUtils.getParentNodeFrom(e.currentTarget, CONFIG.DATA_IDTF.CALENDAR);
    onPressAppt(ApptBooking_objectSpread(ApptBooking_objectSpread({}, value), {}, {
      top: position.top,
      left: position.left
    }));

    // handle when mouse is outside a component while mouse-down is still happening and suddenly mouse-up comes in
    document.addEventListener("mouseup", handleMouseOutsideWhileDragging, {
      once: true
    });
  };
  var onDragging = (0,react.useCallback)(function () {
    if (isDragRef.current) {
      var currScrollBarTop = (scrollEl === null || scrollEl === void 0 ? void 0 : scrollEl.scrollTop) || 0;
      var curApptTop = position.top;
      ...
      // touch the edge of botoom
        ...
        if (floorTop - autoScrollThresholdRef.current <= topEdgeRef.current && scrollEl) {
          ...
          removeAutoScrollIntervalRef.current = TimeUtils.wrapperSetInterval(function () {
            currScrollBarTop -= CONFIG.SPEED;
            curApptTop -= CONFIG.SPEED;
            setPosition(function (prev) {
              return ApptBooking_objectSpread(ApptBooking_objectSpread({}, prev), {}, {
                top: curApptTop
              });
            });
  ...
  var onEndDragging = function onEndDragging(e) {
    // if still false means the user is not drag at all, just click
    if (!isDragRef.current) preventDragEventRef.current && preventDragEventRef.current();else {
      onReleaseAppt(value.id, startTime, value.duration);
      updateDraggingState(false);
    }
  };
  ...
  /* handle resize event */
  var updateResizeState = function updateResizeState(value) {
    if (value) {
  ...
  var handleMouseOutsideWhileResizing = function handleMouseOutsideWhileResizing(e) {
    e.stopPropagation();
    if (isResizeRef.current) {
      var newDuration = TimeUtils.convertHeightToDuration(sizeBackerRef.current + 1, CONFIG.CSS.LINE_HEIGHT, calendarState.duration);
      onReleaseAppt(value.id, startTime, newDuration);
      updateResizeState(false);
    }
  };
  var onStartResize = function onStartResize(e) {
    ...
    // handle when mouse is outside a component while mouse-down is still happening and suddenly mouse-up comes in
    document.addEventListener("mouseup", handleMouseOutsideWhileResizing, {
      once: true
    });
  };
  var onResizing = (0,react.useCallback)(function () {
    if (isResizeRef.current) {
      var distance = lastMouseTopPositionRef.current - mousePosition.top;
      var newHeight = value.height - distance;
      sizeBackerRef.current = newHeight;
      setSize(function (s) {
        return ApptBooking_objectSpread(ApptBooking_objectSpread({}, s), {}, {
          height: newHeight
        });
      });
    }
  }, [mousePosition.top, value.height]);
  var onEndResize = function onEndResize(e) {
    e.stopPropagation(); // prevent to fire a drag event on parent div

    if (isResizeRef.current) {
      var newDuration = TimeUtils.convertHeightToDuration(size.height + 1, CONFIG.CSS.LINE_HEIGHT, calendarState.duration);
      onReleaseAppt(value.id, startTime, newDuration);
      updateResizeState(false);
    }
  };
  ...
  return /*#__PURE__*/react.createElement(ApptBooking_Wrapper, {
    "data-idtf": CONFIG.DATA_IDTF.APPT_BOOKING,
    $status: value.status,
    style: {
      transform: "translateX(".concat(updatedLeft, "px) translateY(").concat(position.top, "px)"),
      width: updatedWidth,
...
/* harmony default export */ const app_ApptBooking = (ApptBooking);
;// CONCATENATED MODULE: ./src/app/ApptClone.tsx
var ApptClone_templateObject, ApptClone_templateObject2;
function ApptClone_taggedTemplateLiteral(strings, raw) { if (!raw) { raw = strings.slice(0); } return Object.freeze(Object.defineProperties(strings, { raw: { value: Object.freeze(raw) } })); }
...
var ApptClone = function ApptClone(_ref) {
  ...
  var endTime = TimeUtils.convertSecondsToHourString(value.endTime);
  return /*#__PURE__*/react.createElement(ApptClone_Wrapper, {
    "data-idtf": CONFIG.DATA_IDTF.APPT_CLONE,
    style: {
      transform: "translateX(".concat(value.left, "px) translateY(").concat(value.top, "px)"),
      width: value.width,
      height: value.height
    }
  }, /*#__PURE__*/react.createElement(ApptClone_Content, {
    $dir: "column"
  }, /*#__PURE__*/react.createElement("div", null, startTime, "-", endTime), /*#__PURE__*/react.createElement("div", null, value.title), /*#__PURE__*/react.createElement("div", null, value.content)));
};
/* harmony default export */ const app_ApptClone = (ApptClone);
;// CONCATENATED MODULE: ./src/app/Appointment.tsx
function Appointment_typeof(o) { "@babel/helpers - typeof"; return Appointment_typeof = "function" == typeof Symbol && "symbol" == typeof Symbol.iterator ? function (o) { return typeof o; } : function (o) { return o && "function" == typeof Symbol && o.constructor === Symbol && o !== Symbol.prototype ? "symbol" : typeof o; }, Appointment_typeof(o); }
var Appointment_templateObject;
function Appointment_ownKeys(e, r) { var t = Object.keys(e); if (Object.getOwnPropertySymbols) { var o = Object.getOwnPropertySymbols(e); r && (o = o.filter(function (r) { return Object.getOwnPropertyDescriptor(e, r).enumerable; })), t.push.apply(t, o); } return t; }
function Appointment_objectSpread(e) { for (var r = 1; r < arguments.length; r++) { var t = null != arguments[r] ? arguments[r] : {}; r % 2 ? Appointment_ownKeys(Object(t), !0).forEach(function (r) { Appointment_defineProperty(e, r, t[r]); }) : Object.getOwnPropertyDescriptors ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(t)) : Appointment_ownKeys(Object(t)).forEach(function (r) { Object.defineProperty(e, r, Object.getOwnPropertyDescriptor(t, r)); }); } return e; }
function Appointment_defineProperty(obj, key, value) { key = Appointment_toPropertyKey(key); if (key in obj) { Object.defineProperty(obj, key, { value: value, enumerable: true, configurable: true, writable: true }); } else { obj[key] = value; } return obj; }
function Appointment_toPropertyKey(arg) { var key = Appointment_toPrimitive(arg, "string"); return Appointment_typeof(key) === "symbol" ? key : String(key); }
function Appointment_toPrimitive(input, hint) { if (Appointment_typeof(input) !== "object" || input === null) return input; var prim = input[Symbol.toPrimitive]; if (prim !== undefined) { var res = prim.call(input, hint || "default"); if (Appointment_typeof(res) !== "object") return res; throw new TypeError("@@toPrimitive must return a primitive value."); } return (hint === "string" ? String : Number)(input); }
function _toConsumableArray(arr) { return _arrayWithoutHoles(arr) || _iterableToArray(arr) || Appointment_unsupportedIterableToArray(arr) || _nonIterableSpread(); }
function _nonIterableSpread() { throw new TypeError("Invalid attempt to spread non-iterable instance.\nIn order to be iterable, non-array objects must have a [Symbol.iterator]() method."); }
function _iterableToArray(iter) { if (typeof Symbol !== "undefined" && iter[Symbol.iterator] != null || iter["@@iterator"] != null) return Array.from(iter); }
function _arrayWithoutHoles(arr) { if (Array.isArray(arr)) return Appointment_arrayLikeToArray(arr); }
function Appointment_slicedToArray(arr, i) { return Appointment_arrayWithHoles(arr) || Appointment_iterableToArrayLimit(arr, i) || Appointment_unsupportedIterableToArray(arr, i) || Appointment_nonIterableRest(); }
function Appointment_nonIterableRest() { throw new TypeError("Invalid attempt to destructure non-iterable instance.\nIn order to be iterable, non-array objects must have a [Symbol.iterator]() method."); }
function Appointment_unsupportedIterableToArray(o, minLen) { if (!o) return; if (typeof o === "string") return Appointment_arrayLikeToArray(o, minLen); var n = Object.prototype.toString.call(o).slice(8, -1); if (n === "Object" && o.constructor) n = o.constructor.name; if (n === "Map" || n === "Set") return Array.from(o); if (n === "Arguments" || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(n)) return Appointment_arrayLikeToArray(o, minLen); }
function Appointment_arrayLikeToArray(arr, len) { if (len == null || len > arr.length) len = arr.length; for (var i = 0, arr2 = new Array(len); i < len; i++) arr2[i] = arr[i]; return arr2; }
function Appointment_iterableToArrayLimit(r, l) { var t = null == r ? null : "undefined" != typeof Symbol && r[Symbol.iterator] || r["@@iterator"]; if (null != t) { var e, n, i, u, a = [], f = !0, o = !1; try { if (i = (t = t.call(r)).next, 0 === l) { if (Object(t) !== t) return; f = !1; } else for (; !(f = (e = i.call(t)).done) && (a.push(e.value), a.length !== l); f = !0); } catch (r) { o = !0, n = r; } finally { try { if (!f && null != t["return"] && (u = t["return"](), Object(u) !== u)) return; } finally { if (o) throw n; } } return a; } }
function Appointment_arrayWithHoles(arr) { if (Array.isArray(arr)) return arr; }
function Appointment_taggedTemplateLiteral(strings, raw) { if (!raw) { raw = strings.slice(0); } return Object.freeze(Object.defineProperties(strings, { raw: { value: Object.freeze(raw) } })); }
...
var Appointment_Wrapper = st.div(Appointment_templateObject || (Appointment_templateObject = Appointment_taggedTemplateLiteral(["\n  position: absolute;\n  top: 0;\n  left: 0;\n  bottom: 0;\n  right: 0;\n  z-index: 2;\n"])));
var Appointment = function Appointment(_ref) {
  ...
  var _useState = (0,react.useState)(null),
    _useState2 = Appointment_slicedToArray(_useState, 2),
    apptClone = _useState2[0],
    setApptClone = _useState2[1];
  var cloneBackerRef = (0,react.useRef)({});
  var isPressRef = (0,react.useRef)(false);
  var dateline = DateUtils.getDateline(calendarState.currentDate, calendarState.viewMode);
  var renderByDayMode = function renderByDayMode(apptProp) {
    var apptByDate = apptProp.filter(function (a) {
      return DateUtils.isEqual(calendarState.currentDate, a.createdAt);
    });
    var apptReordered = AppointmentUtils.layoutAlgorithm(apptByDate, {
  ...
  var renderByWeekMode = function renderByWeekMode(apptProp) {
    var apptReordered = [];

    // iterate over the array to sort the appointment's "createdAt" attribute relative to the column
    var apptByWeek = [];
    dateline.forEach(function (d) {
      var apptCell = [];
      apptProp.forEach(function (a) {
        var res = DateUtils.isEqual(d.origin, a.createdAt);
    ...
    apptByWeek.forEach(function (ca, i) {
      var apptReodered = AppointmentUtils.layoutAlgorithm(ca, {
        daytimeStart: calendarState.dayTime.start,
        duration: calendarState.duration,
        columnWidth: widthTimeline,
        weekcolumnIndex: i
      });
      apptReordered.push.apply(apptReordered, _toConsumableArray(apptReodered));
    });
    return apptReordered;
  };
  var render = function render(apptProp) {
    var apptRender = [];
    if (calendarState.viewMode === "DAY") apptRender = renderByDayMode(apptProp);else if (calendarState.viewMode === "WEEK") apptRender = renderByWeekMode(apptProp);
    return apptRender;
  };
  var onPressApptBooking = function onPressApptBooking(value) {
    if (value.id) {
      setApptClone(value);
  ...
  var onReleaseApptBooking = function onReleaseApptBooking(id, startTime, duration) {
    if (apptClone && id && apptClone.id === id) {
      setApptClone(null);
    }
    var weekColIdx = Math.round(mousePosition.left / widthTimeline);
    var dayCustom = dateline[weekColIdx];
    var payload = {
      startTime: startTime,
      duration: duration,
      createdAt: dayCustom.origin
    };
    var apptCopy = calendarState.appointments.slice();
    for (var i = 0; i < apptCopy.length; i++) {
      if (id === apptCopy[i].id) {
        apptCopy[i] = Appointment_objectSpread(Appointment_objectSpread({}, apptCopy[i]), payload);
        break;
      }
    }
    dispath({
      type: EAction.UPDATE_APPT,
      payload: apptCopy
    });
  ...
  var onMouseLeave = function onMouseLeave(e) {
    return setApptClone(null);
  };
  return /*#__PURE__*/react.createElement(Appointment_Wrapper, {
    "data-idtf": CONFIG.DATA_IDTF.APPOINTMENT,
    onMouseEnter: onMouseEnter,
    onMouseLeave: onMouseLeave
  }, apptClone && /*#__PURE__*/react.createElement(app_ApptClone, {
    value: apptClone
  }), render(calendarState.appointments).map(function (appt) {
    return /*#__PURE__*/react.createElement(app_ApptBooking, {
      key: appt.id,
      value: appt,
      scrollEl: scrollEl,
...
/* harmony default export */ const app_Appointment = (Appointment);
;// CONCATENATED MODULE: ./src/app/Ghost.tsx
var Ghost_templateObject;
function Ghost_taggedTemplateLiteral(strings, raw) { if (!raw) { raw = strings.slice(0); } return Object.freeze(Object.defineProperties(strings, { raw: { value: Object.freeze(raw) } })); }
...
var Ghost = function Ghost(_ref) {
  var rect = _ref.rect,
    timeEachCell = _ref.timeEachCell;
  return /*#__PURE__*/react.createElement(Ghost_Wrapper, {
    "data-idtf": CONFIG.DATA_IDTF.GHOST,
    $rect: rect
  }, timeEachCell);
};
/* harmony default export */ const app_Ghost = (Ghost);
;// CONCATENATED MODULE: ./src/app/Row.tsx
...
var Row = function Row() {
  var calendarState = useCalendarState();
  var renderRow = (0,react.useCallback)(function () {
    return TimeUtils.createTimes(calendarState.dayTime.end, calendarState.dayTime.start, calendarState.duration).map(function (t, i) {
      var currentTime = calendarState.dayTime.start + i * calendarState.duration;
      var workingTime = TimeUtils.checkWorkingTime(calendarState.dayTime, calendarState.workingTime, currentTime);
      var groupTime = TimeUtils.checkGroupTime(calendarState.groupTime, calendarState.duration, i, "top");
      var classname = clsx({
        wt: workingTime,
        gt: groupTime
      });
  ...
  return /*#__PURE__*/react.createElement("div", {
    "data-idtf": CONFIG.DATA_IDTF.ROW
  }, renderRow());
};
/* harmony default export */ const app_Row = (Row);
;// CONCATENATED MODULE: ./src/app/Grid.tsx
...
function Grid_taggedTemplateLiteral(strings, raw) { if (!raw) { raw = strings.slice(0); } return Object.freeze(Object.defineProperties(strings, { raw: { value: Object.freeze(raw) } })); }
...
var Grid = function Grid() {
  ...
  var _useState = (0,react.useState)(0),
    ...
    gridWidth = _useState2[0],
    setGridWidth = _useState2[1];
  var _useState3 = (0,react.useState)(null),
    _useState4 = Grid_slicedToArray(_useState3, 2),
    scrollEl = _useState4[0],
    setScrollEl = _useState4[1];
  var _useState5 = (0,react.useState)(""),
    _useState6 = Grid_slicedToArray(_useState5, 2),
    timeEachCell = _useState6[0],
    setTimeEachCell = _useState6[1];
  ...
  var _useState9 = (0,react.useState)(false),
    _useState10 = Grid_slicedToArray(_useState9, 2),
    isShowGhost = _useState10[0],
    setShowGhost = _useState10[1];
  var dateline = DateUtils.getDateline(calendarState.currentDate, calendarState.viewMode);
  var widthTimeline = gridWidth / dateline.length;
  ...
  */
  var onMouseMove = function onMouseMove(e) {
    ...
    var dataIdtf = e.target.getAttribute(CONFIG.DATA_IDTF.THIS);
    if (dataIdtf === CONFIG.DATA_IDTF.APPT_BOOKING || dataIdtf === CONFIG.DATA_IDTF.APPT_RESIZE || calendarState.isFireEvent) {
      setShowGhost(false);
    }
    var topOutside = ElementUtils.getOffsetToDocument(gridRef.current, "top");
    var leftOutside = ElementUtils.getOffsetToDocument(gridRef.current, "left");
    var offsetTop = e.pageY - topOutside + ((scrollEl === null || scrollEl === void 0 ? void 0 : scrollEl.scrollTop) || 0);
    var offsetLeft = e.pageX - leftOutside + ((scrollEl === null || scrollEl === void 0 ? void 0 : scrollEl.scrollLeft) || 0);
    var lineIdx = Math.floor(offsetTop / CONFIG.CSS.LINE_HEIGHT);
    var colIdx = Math.floor(offsetLeft / widthTimeline);
    var top = lineIdx * CONFIG.CSS.LINE_HEIGHT;
    var left = colIdx * widthTimeline;
    var seconds = lineIdx * calendarState.duration + calendarState.dayTime.start;
    var time = TimeUtils.convertSecondsToHourString(seconds, calendarState.timeType);
    setPosition({
      top: top,
      left: left,
      pageY: e.pageY,
  ...
  (0,react.useEffect)(function () {
    if (gridRef && gridRef.current && gridRef.current.parentElement) {
      ...
      var _scrollEl = ElementUtils.getParentNodeFrom(gridRef.current, CONFIG.DATA_IDTF.SCROLLING);
      setScrollEl(_scrollEl);
    }
  }, []);
  return /*#__PURE__*/react.createElement(Grid_Wrapper, {
    "data-idtf": CONFIG.DATA_IDTF.GRID,
    onMouseMove: onMouseMove,
    onMouseLeave: onMouseLeave,
    onContextMenu: onRightClick,
    ref: gridRef
  }, /*#__PURE__*/react.createElement(app_NowIndicator, {
    type: "LINE",
    widthTimeline: widthTimeline
  }), isShowGhost && /*#__PURE__*/react.createElement(app_Ghost, {
    timeEachCell: timeEachCell,
    rect: {
      top: position.top,
      left: position.left,
      width: widthTimeline,
      height: CONFIG.CSS.LINE_HEIGHT
    }
  }), /*#__PURE__*/react.createElement(app_Appointment, {
    scrollEl: scrollEl,
    widthTimeline: widthTimeline,
    mousePosition: position
  }), /*#__PURE__*/react.createElement(app_Row, null));
};
/* harmony default export */ const app_Grid = (Grid);
;// CONCATENATED MODULE: ./src/app/Swipable.tsx
var Swipable_templateObject;
function Swipable_taggedTemplateLiteral(strings, raw) { if (!raw) { raw = strings.slice(0); } return Object.freeze(Object.defineProperties(strings, { raw: { value: Object.freeze(raw) } })); }





var Swipable_Wrapper = st.div(Swipable_templateObject || (Swipable_templateObject = Swipable_taggedTemplateLiteral(["\n  flex: 1;\n  height: max-content;\n"])));
var Swipable = function Swipable() {
  return /*#__PURE__*/react.createElement(Swipable_Wrapper, {
    "data-idtf": CONFIG.DATA_IDTF.SWIPABLE
  }, /*#__PURE__*/react.createElement(app_Dateline, null), /*#__PURE__*/react.createElement(app_Grid, null));
};
/* harmony default export */ const app_Swipable = (Swipable);
;// CONCATENATED MODULE: ./src/app/Time.tsx
var Time_templateObject;
function Time_taggedTemplateLiteral(strings, raw) { if (!raw) { raw = strings.slice(0); } return Object.freeze(Object.defineProperties(strings, { raw: { value: Object.freeze(raw) } })); }
...
var Time_Wrapper = st.div(Time_templateObject || (Time_templateObject = Time_taggedTemplateLiteral(["\n  position: relative;\n"])));
var Time = function Time() {
  var calendarState = useCalendarState();
  var renderTime = (0,react.useCallback)(function () {
    return TimeUtils.createTimes(calendarState.dayTime.end, calendarState.dayTime.start, calendarState.duration).map(function (t, i) {
      var currentTime = calendarState.dayTime.start + i * calendarState.duration;
      var time = TimeUtils.convertSecondsToHourString(currentTime);
      var showTime = TimeUtils.displayTime(currentTime, calendarState.dayTime.start, calendarState.duration * CONFIG.MAPPING_TIME[calendarState.duration]);
      var classname = clsx({
        ngt: true
      });
      return /*#__PURE__*/react.createElement(Line, {
  ...
  return /*#__PURE__*/react.createElement(Time_Wrapper, {
    "data-idtf": CONFIG.DATA_IDTF.TIME
  }, /*#__PURE__*/react.createElement(app_NowIndicator, {
    type: "PILL"
  }), renderTime());
};
/* harmony default export */ const app_Time = (Time);
;// CONCATENATED MODULE: ./src/app/Interval.tsx
var Interval_templateObject, Interval_templateObject2;
function Interval_taggedTemplateLiteral(strings, raw) { if (!raw) { raw = strings.slice(0); } return Object.freeze(Object.defineProperties(strings, { raw: { value: Object.freeze(raw) } })); }




var Interval_Wrapper = st.div(Interval_templateObject || (Interval_templateObject = Interval_taggedTemplateLiteral(["\n  width: 48px;\n  height: max-content;\n"])));
...
var Interval = function Interval() {
  ...
  }, /*#__PURE__*/react.createElement(Box, null), /*#__PURE__*/react.createElement(app_Time, null));
};
/* harmony default export */ const app_Interval = (Interval);
;// CONCATENATED MODULE: ./src/app/Scrolling.tsx
var Scrolling_templateObject;
function Scrolling_taggedTemplateLiteral(strings, raw) { if (!raw) { raw = strings.slice(0); } return Object.freeze(Object.defineProperties(strings, { raw: { value: Object.freeze(raw) } })); }
...
var Scrolling_Wrapper = st(Flex)(Scrolling_templateObject || (Scrolling_templateObject = Scrolling_taggedTemplateLiteral(["\n  overflow-y: auto;\n  overflow-x: hidden;\n  -webkit-overflow-scrolling: touch;\n"])));
var Scrolling = function Scrolling() {
  return /*#__PURE__*/react.createElement(Scrolling_Wrapper, {
    "data-idtf": CONFIG.DATA_IDTF.SCROLLING
...
function Calendar_typeof(o) { "@babel/helpers - typeof"; return Calendar_typeof = "function" == typeof Symbol && "symbol" == typeof Symbol.iterator ? function (o) { return typeof o; } : function (o) { return o && "function" == typeof Symbol && o.constructor === Symbol && o !== Symbol.prototype ? "symbol" : typeof o; }, Calendar_typeof(o); }
var Calendar_templateObject;
...
function Calendar_ownKeys(e, r) { var t = Object.keys(e); if (Object.getOwnPropertySymbols) { var o = Object.getOwnPropertySymbols(e); r && (o = o.filter(function (r) { return Object.getOwnPropertyDescriptor(e, r).enumerable; })), t.push.apply(t, o); } return t; }
function Calendar_objectSpread(e) { for (var r = 1; r < arguments.length; r++) { var t = null != arguments[r] ? arguments[r] : {}; r % 2 ? Calendar_ownKeys(Object(t), !0).forEach(function (r) { Calendar_defineProperty(e, r, t[r]); }) : Object.getOwnPropertyDescriptors ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(t)) : Calendar_ownKeys(Object(t)).forEach(function (r) { Object.defineProperty(e, r, Object.getOwnPropertyDescriptor(t, r)); }); } return e; }
...
function Calendar_taggedTemplateLiteral(strings, raw) { if (!raw) { raw = strings.slice(0); } return Object.freeze(Object.defineProperties(strings, { raw: { value: Object.freeze(raw) } })); }
 ...
 */
var Calendar = function Calendar(userProps) {
  dayjs_min_default().locale(userProps.locale);

  /* modified userprops with some necessary value for app */
  var apptCopy = userProps.appointments.slice();
  var modifiedAppt = apptCopy.map(function (appt) {
    return Calendar_objectSpread(Calendar_objectSpread({}, appt), {}, {
      startTime: appt.startTime * CONFIG.SECONDS_PER_HOUR
    });
  });
  var appProps = Calendar_objectSpread(Calendar_objectSpread({}, userProps), {}, {
    ...
    appointments: Calendar_toConsumableArray(modifiedAppt),
    todayGlobalIns: dayjs_min_default()(),
    currentDate: dayjs_min_default()(),
    isFireEvent: false
  });
  return /*#__PURE__*/react.createElement(useCalendarContext, {
...
root.render( /*#__PURE__*/react.createElement(react.StrictMode, null, /*#__PURE__*/react.createElement("div", {
  style: {
    width: "100vw",
    height: "100vh",
    padding: "24px"
  }
}, /*#__PURE__*/react.createElement(app_Calendar, {
  duration: 30,
  timeType: 24,
  groupTime: 60,
  nowIndicator: true,
  ...
  apptChange: function apptChange(appts) {
    console.log(appts);
  },
  apptClick: function apptClick(appt) {
    console.log(appt);
  }
}))));
...
/******/ 	(() => {
/******/ 		var inProgress = {};
/******/ 		var dataWebpackPrefix = "react-cool-calendar:";
...
/******/ 		}
/******/ 		
/******/ 		self["webpackHotUpdatereact_cool_calendar"] = (chunkId, moreModules, runtime) => {
